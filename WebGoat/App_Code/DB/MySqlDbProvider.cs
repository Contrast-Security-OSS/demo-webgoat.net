...
namespace OWASP.WebGoat.NET.App_Code.DB
{
    public class MySqlDbProvider : IDbProvider
    {
        ...
        public MySqlDbProvider(IConfig config)
        {
            if (config == null)
                _connectionString = string.Empty;
                
            if (!string.IsNullOrEmpty(config.Get(DbConstants.KEY_PWD)))
            {
            ...
            _uid = config.Get(DbConstants.KEY_UID);
            _pwd = config.Get(DbConstants.KEY_PWD);
            _database = config.Get(DbConstants.KEY_DATABASE);
            _host = config.Get(DbConstants.KEY_HOST);
            _clientExec = config.Get(DbConstants.KEY_CLIENT_EXEC);
            _port = config.Get(DbConstants.KEY_PORT);
        }
