...
namespace OWASP.WebGoat.NET.App_Code.DB
{
    public class SqliteDbProvider : IDbProvider
    {
        ...
        public SqliteDbProvider(IConfig config)
        {
            _connectionString = string.Format("Data Source={0};Version=3", config.Get(DbConstants.KEY_FILE_NAME));

            _clientExec = config.Get(DbConstants.KEY_CLIENT_EXEC);
            _dbFileName = config.Get(DbConstants.KEY_FILE_NAME);
